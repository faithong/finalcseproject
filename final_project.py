...
with col1:
    ...
    st.caption("There are social and economic factors that \
                influence the rate of lung cancer in different communities.\
                However, despite racial disparities in lung cancer, genomic\
                level analysis of lung cancer in underserved patients of\
                African descent remains underexplored.")
...
visualization = st.selectbox(
     "",
     ('About the Dataset',
      'How Does the Average APA Site in African Americans\
       Compare to that of European Americans?',
      'How does the average PSI index in African Americans\
       compare to that of European Americans?',
      'Are there any commonalities between amino acid\
       sequences among African American vs. European Americans?',
      'Length of Amino Acids'))

st.markdown("***")
st.text("")

if visualization == 'How Does the Average APA Site in African\
    ...
    all_races['gene_name_class'] = all_races['gene_name']
    + all_races['gene_class']
    ...
    x, y = zip(*lists)
    plt.bar(x, y)
    plt.title('Amino Acids and Codons Differences in African\
    ...
    with col4:
        ...
        st.caption("Here I will include a short explanation\
                    of what this graph shows.")
        st.write()
        gene_biotype_graph = sns.catplot(data=all_races, x="gene_biotype",
                                         kind="count", ci=None)
    ...
    with col5:
        ...
        st.caption("Here I will include a short explanation\
                    of what this graph shows.")
        pair_type_graph = sns.catplot(data=all_races, x="pair_type",
                                      kind="count", ci=None)
    ...
    with col6:
        ...
        st.caption("Here I will include a short explanation of\
                    what this graph shows.")
        gene_class_graph = sns.catplot(data=all_races, x="gene_class",
                                       kind="count", ci=None)
